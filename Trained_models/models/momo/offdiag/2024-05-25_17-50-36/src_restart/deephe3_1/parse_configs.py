...
class TrainConfig(BaseConfig):
    def __init__(self, config_file):
        ...
        if self.checkpoint_dir:
            ...
            if self.use_new_hypp:
                print(f'Overwriting sections [target] and [network] using train config in checkpoint: {config1_path}')
            else:
    ...
    def get_train_section(self):
        ...
        self.min_loss = self._config.getfloat('train', 'min_loss')
    ...
    def get_target_section(self):
        # target
        orbital_types = self._config.get('target', 'orbital_types')
        if orbital_types:
            self.orbital_types = eval(orbital_types)
        else:

...
class MaskMSELoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if torch.sum(mask).item() == 0:
            return torch.tensor(0, dtype=input.dtype, requires_grad=True)
        else:
...
def patch_old_model(src_path):
    if os.path.isfile(f'{src_path}/deephe3_1/version'):
        # Is new model, no need for patching.
        return
    if os.path.isdir(f'{src_path}/deephe3_1'):
        dirname = 'deephe3_1'
    elif os.path.isdir(f'{src_path}/e3deeph_1'):
        dirname = 'e3deeph_1'
    if os.path.isfile(f'{src_path}/{dirname}/model_old.py'):
        # Model already patched, no need for patching
        print('Found older version of DeepH-E3 model with patches already applied')
        return
    print('Found older version of DeepH-E3 model, applying patches')
    os.rename(f'{src_path}/{dirname}/model.py', f'{src_path}/{dirname}/model_old.py')
    with open(f'{src_path}/dataset_info.json') as f:
        dataset_info = json.load(f)
    with open(f'{src_path}/{dirname}/model_old.py', 'r') as f:
        modelfile = f.read()
    head  = f'import torch\n'
    head += f'index_to_Z = torch.tensor({dataset_info["index_to_Z"]})\n'
    head += f'Z_to_index = torch.full((100,), -1, dtype=torch.int64)\n'
    head += f'Z_to_index[index_to_Z] = torch.arange(len(index_to_Z))\n'
    head += f'\n\n'
    modelfile = head + modelfile
    modelfile = modelfile.replace('data.x', 'Z_to_index.to(data.x.device)[data.x]')
    with open(f'{src_path}/{dirname}/model.py', 'w') as f:
        f.write(modelfile)
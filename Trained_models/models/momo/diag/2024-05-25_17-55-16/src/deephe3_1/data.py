...
├── ...
    ...
    def process_worker(self, folder, **kwargs):
        ...
        return get_graph(cart_coords, frac_coords, numbers, stru_id, r=self.radius, max_num_nbr=self.max_num_nbr,
                         ...
                         data_folder=folder, max_num_orbitals=self.max_num_orbitals, 
                         target_file_name=self.target_file_name, inference=self.inference, create_from_DFT=self.create_from_DFT, 
                         only_ij=self.only_ij, **kwargs)
    ...
    def process(self):
        ...
        index_to_Z, Z_to_index, orbital_types_new, max_norb = self.element_statistics(folder_list)
        self.max_num_orbitals = max_norb
    ...
    def element_statistics(self, folder_list):
        elements_all = []
        for folder in folder_list:
            elements = np.loadtxt(os.path.join(folder, 'element.dat'), dtype=int)
            if len(elements.shape) == 0:
                elements = elements[None]
            elements_all.extend(elements.tolist())
        elements_all = torch.tensor(elements_all)
        index_to_Z, inverse_indices = torch.unique(elements_all, sorted=True, return_inverse=True)
        ...
        for i in range(len(index_to_Z)):
            atom_Z = index_to_Z[i].numpy()
            orbital_types_thiselem = None
            for folder in folder_list:
                elements = np.loadtxt(os.path.join(folder, 'element.dat'))
                idx_thiselem = np.where(elements == atom_Z)[0]
                if len(idx_thiselem) > 0:
                    _, orbital_types = load_orbital_types(path=os.path.join(folder, 'orbital_types.dat'), return_orbital_types=True)
                    for idx in idx_thiselem:
                        if orbital_types_thiselem is None:
                            orbital_types_thiselem = orbital_types[idx]
                        else:
                            assert orbital_types_thiselem == orbital_types[idx], 'Orbital types inconsistent between structures'
            assert orbital_types_thiselem is not None
            orbital_types_new.append(orbital_types_thiselem)
            
        atom_num_orbital = [sum(map(lambda x: 2 * x + 1, atom_orbital_types)) for atom_orbital_types in orbital_types_new]
        max_norb = max(atom_num_orbital)
            
        # for data in data_list:
        #     data.x = Z_to_index[data.x]

        return index_to_Z, Z_to_index, orbital_types_new, max_norb
    ...
    def set_mask(self, targets, del_Aij=True, convert_to_net=False):
        ...
        for data in self:
            ...
            if data.Aij is None: raise NotImplementedError # Do not call set_mask when inference!
            ...
            for index_out, equivariant_block in enumerate(equivariant_blocks):
                for N_M_str, block_slice in equivariant_block.items():
                    ...
                    condition_index_edge = torch.where(
                        (atomic_number_edge_i == condition_atomic_number_i)
                        & (atomic_number_edge_j == condition_atomic_number_j)
                    )
                    condition_index_edge = condition_index_edge[0]
                    if len(condition_index_edge) == 0 : continue
                    condition_slice_i = slice(block_slice[0], block_slice[1])
                    condition_slice_j = slice(block_slice[2], block_slice[3])
                    if spinful:
                        cond_idx_i = self.info['Z_to_index'][condition_atomic_number_i]
                        cond_idx_j = self.info['Z_to_index'][condition_atomic_number_j]
                        condition_slice_i_ds = slice(atom_num_orbital[cond_idx_i] + block_slice[0],
                                                     atom_num_orbital[cond_idx_i] + block_slice[1]) # ds = down spin
                        condition_slice_j_ds = slice(atom_num_orbital[cond_idx_j] + block_slice[2],
                                                     atom_num_orbital[cond_idx_j] + block_slice[3])
                    out_slice = slice(out_slices[index_out], out_slices[index_out + 1])
                    if spinful:
                        # noncollinear spin block order:
                        # 0(uu) 1(ud)
                        # 2(du) 3(dd)
                        label[condition_index_edge, 0, out_slice] += data.Aij[condition_index_edge, condition_slice_i, condition_slice_j].reshape(len(condition_index_edge), -1)
                        label[condition_index_edge, 1, out_slice] += data.Aij[condition_index_edge, condition_slice_i, condition_slice_j_ds].reshape(len(condition_index_edge), -1)
                        label[condition_index_edge, 2, out_slice] += data.Aij[condition_index_edge, condition_slice_i_ds, condition_slice_j].reshape(len(condition_index_edge), -1)
                        label[condition_index_edge, 3, out_slice] += data.Aij[condition_index_edge, condition_slice_i_ds, condition_slice_j_ds].reshape(len(condition_index_edge), -1)
                        mask[condition_index_edge, :, out_slice] += 1
                    else:
                        label[condition_index_edge, out_slice] += data.Aij[condition_index_edge, condition_slice_i, condition_slice_j].reshape(len(condition_index_edge), -1)
                        mask[condition_index_edge, out_slice] += 1
            if del_Aij:
                del data.Aij_mask
            if data.Aij is not None:

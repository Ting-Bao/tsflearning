    ...
    def error_plot(self, include=[], exclude=[], mode='mae'):
        ...
        with h5py.File(self.h5file, 'r') as f:
            ...
            for i in range(num_element):
                num_orbitals[i] = np.sum(2*np.array(self.dataset_info.orbital_types[i])+1)
            errors = [[None for _ in range(num_element)] for _ in range(num_element)]
            for i in range(num_element):
                for j in range(num_element):
                    norb1 = num_orbitals[i] * (1+spinful)
                    norb2 = num_orbitals[j] * (1+spinful)
                    errors[i][j] = np.zeros((norb1, norb2))
            ...
            for s in included:
                ...
                for i in range(num_element):
                    for j in range(num_element):
                        ...
                        if np.all(~selector): continue # no such hopping in structure
                        ...
                        error = self.kernel.update_hopping({}, error[None, ...], self.dataset_info.index_to_Z[node_attr], edge_index[:, dummy_ix, None], np.array([0]), debug=True)['0']

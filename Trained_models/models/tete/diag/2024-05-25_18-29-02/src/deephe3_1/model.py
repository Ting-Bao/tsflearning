...
class Net(nn.Module):
    def __init__(self, index_to_Z, irreps_embed_node, irreps_edge_init, irreps_sh, irreps_mid_node, 
        ...
        index_to_Z = torch.tensor(index_to_Z)
        ...
        self.sigma = torch.nn.Parameter(torch.ones(num_species**2+num_species, irreps_out_edge.num_irreps))
        self.mu = torch.nn.Parameter(torch.zeros(num_species**2+num_species, irreps_out_edge.count("0e")))
        self.sigma_extract = []
        self.mu_inv_extract = []
        index_sigma = 0
        index_mu = 0
        for mul, ir in irreps_out_edge:
            for _ in range(mul):
                self.sigma_extract += [index_sigma for _ in range(ir.dim)]
                index_sigma += 1
                if ir == Irrep("0e"):
                    self.mu_inv_extract.append(index_mu)
                index_mu += ir.dim
        assert len(self.sigma_extract) == irreps_out_edge.dim
        assert len(self.mu_inv_extract) == irreps_out_edge.count("0e")

    def forward(self, data):
        node_index = self.Z_to_index.to(data.x.device)[data.x]
        node_one_hot = F.one_hot(node_index, num_classes=self.num_species).type(torch.get_default_dtype())
        edge_one_hot = F.one_hot(self.num_species * node_index[data.edge_index[0]] + node_index[data.edge_index[1]],
        ...
        selfloop_edge = torch.abs(data["edge_attr"][:, 0]) < 1e-7
        sigma = (edge_one_hot@self.sigma[self.num_species:])[:, self.sigma_extract]
        selfloop_sigma = self.sigma[node_index[data.edge_index[0, selfloop_edge]]][:, self.sigma_extract]
        sigma[selfloop_edge] = selfloop_sigma
        mu = torch.zeros_like(edge_fea)
        mu[:, self.mu_inv_extract] = edge_one_hot@self.mu[self.num_species:]
        selfloop_mu = self.mu[node_index[data.edge_index[0, selfloop_edge]]]
        selfloop_all_mu = mu[selfloop_edge]
        selfloop_all_mu[:, self.mu_inv_extract] = selfloop_mu
        mu[selfloop_edge] = selfloop_all_mu

        edge_fea = sigma*edge_fea + mu

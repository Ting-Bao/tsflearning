...
def batch_file_output(poscar, super, shift, type, num_pert, pert, num_atom_each_layer, shift_which_layer, basis_accu, soc, dftu, dftuval, magmom, widespace = False):
    ...
    #### shift the whole layer, i.e. interlayer space, sample with mu and sigma
    mu = 0 # center is the origin interlayer space
    sigma = 0.2 # specially, beyond 2 sigma will be dropped in the sampling       
    ...
    for shift_idx_1 in range(shift[0]):
        for shift_idx_2 in range(shift[1]):
            for pert_idx in range(num_pert):
                ...
                displace_interlayer = 999.999
                while abs(displace_interlayer)>2*sigma:
                    displace_interlayer = np.random.normal(loc=mu,scale=sigma)
                
                # baot: for ZhangYang case only, remove when for other use
                displace_interlayer += 0.0
                ...
                with open(os.path.join(folder_path, 'shift.json'), 'w') as shift_f:
                    json.dump({'displace_interlayer':displace_interlayer,'displace_a': shift_list[struct_idx][0], 'displace_b': shift_list[struct_idx][1]}, shift_f, indent=4)

                in_file = openmx_input(struct, basis_accu, soc, dftu, dftuval, magmom)
                with open(openmx_in_file, 'w') as f:
                    f.write(in_file)
    
    layer_displace_list = [i + 7.212 for i in layer_displace_list] # input the origin distance
    lower_bound = 6.4
    upper_bound = 8.0
    bin_width = 0.1  # Set the desired width of each bin
...
def move_layer(struct,displacement):
    ...
    work for bilayer only
    """
    upperlayer = []
    cart_coords_z = struct.cart_coords[:,2]
    idx_coords_z = np.argsort(cart_coords_z) # return original idx of elements in sorted list (from small to large)
    
    upperlayer = idx_coords_z[len(idx_coords_z)//2:]

    for atom_index in upperlayer:
        # Get the atom you want to move
        atom_to_move = struct.sites[atom_index]
        # Move the atom by adding the displacement vector to its coordinates
        new_position = atom_to_move.coords + displacement
        # Update the coordinates of the atom
        atom_to_move.coords = new_position
        # Update the structure with the modified atom position
        struct.sites[atom_index] = atom_to_move
...
if __name__=="__main__":
    ...
    batch_file_output('POSCAR', [6, 6], [23, 23], 1,
                    1, [0.15, 0.15, 0.15], [3, 3], 1,

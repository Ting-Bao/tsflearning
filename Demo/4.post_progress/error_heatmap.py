...
im = ax.imshow(errors, cmap='Blues', vmin = 0.0, vmax = 3.0)
# im = ax.imshow(errors, vmin=-2, cmap='Blues')
...
ax.set_xticklabels(xlabels, fontsize=0.6*fontsize)
ax.set_yticklabels(ylabels, fontsize=0.6*fontsize)
...
ax.set_title(r'MAE of $\rho_{i\alpha, j\beta}$ in selected structure', pad=10, fontsize=1.3*fontsize)
...
plt.tight_layout()
plt.savefig(f'{filename_plot}.png', dpi=240,transparent=True)
plt.savefig(f'{filename_plot}.svg',transparent=True)

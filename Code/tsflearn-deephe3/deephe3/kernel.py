...
from .utils import Logger, RevertDecayLR, MaskMSELoss, LossRecord, process_targets, set_random_seed, flt2cplx, patch_old_model
...
class DatasetInfo:
    ...
    def contains(self, other) -> bool:
        if other.__class__ != __class__:
            raise ValueError
        if other.spinful != self.spinful: return False
        for Z in other.index_to_Z:
            if Z not in self.index_to_Z: return False
        for Z in other.index_to_Z:
            ix_self = self.Z_to_index[Z]
            ix_other = other.Z_to_index[Z]
            if self.orbital_types[ix_self] != other.orbital_types[ix_other]: return False
        return True
...
class NetOutInfo:
    ...
    def merge(self, other):
        ...
        if other.dataset_info.contains(self.dataset_info):
            self.dataset_info = other.dataset_info
        else:
            msg = 'Incompatible datasets'
            assert self.dataset_info.contains(other.dataset_info), msg
...
class DeepHE3Kernel:
    ...
    def train(self, train_config):
        ...
        # if orbital_types is present in train_config, then use it to generate dataset_info
        # instead of reading from dataset
        specified_dinfo = None # todo: move to parseconfig
        if config.orbital_types is not None:
            index_to_Z = sorted(config.orbital_types.keys())
            orbital_types = [config.orbital_types[Z] for Z in index_to_Z]
            specified_dinfo = DatasetInfo(dataset.info['spinful'], index_to_Z, orbital_types)
            assert specified_dinfo.contains(self.dataset_info), 'orbital_types in config is incompatible with dataset'

        net_out_info = self.config_set_target(specified_dinfo if specified_dinfo is not None else self.dataset_info,
        ...
        if config.checkpoint_dir:
            ...
            self.init_normalize(net, train_loader)
            global_step = 0
        ...
        exit_reason = 'it is interrupted'
        try:
            while True:
                if epoch >= config.num_epoch:
                    exit_reason = 'maximum epoch number is reached'
                    break
                elif learning_rate <= config.min_lr:
                    exit_reason = 'learning rate threshold (min_lr) is reached'
                    break
                elif best_loss <= config.min_loss:
                    exit_reason = 'loss threshold (min_loss) is reached'
                    break
        ...
        print(f'\nTraining finished because {exit_reason}.')
    ...
    def eval(self, config, debug=False):
        ...
        if not eval_config.inference:
            os.makedirs(f'{eval_config.out_dir}/test_result/', exist_ok=True)
            h5file = os.path.join(eval_config.out_dir, 'test_result/test_result.h5')
        ...
        for index_model, model_path in enumerate(model_path_list):
            ...
            assert net_out_info.dataset_info.contains(self.dataset_info), 'Dataset incompatible with model'
            net_out_info_1 = self.config_set_target(net_out_info.dataset_info)
            assert net_out_info == net_out_info_1
            self.net_out_info = net_out_info
            if not eval_config.inference:
            ...
            if not 'sigma' in checkpoint['state_dict'].keys():
                checkpoint['state_dict']['sigma'] = model_state_dict["sigma"]
            if not 'mu' in checkpoint['state_dict'].keys():
                checkpoint['state_dict']['mu'] = model_state_dict["mu"]
            net.load_state_dict(checkpoint['state_dict'])
            net.eval()
        ...
        if not eval_config.inference:
            h5_fp.close()
            src = os.path.join(eval_config.out_dir, 'test_result/src')
            os.makedirs(src, exist_ok=True)
            net_out_combined.save_json(os.path.join(eval_config.out_dir, 'test_result/src'))
    ...
    def get_graph(self, config: BaseConfig, inference=False): # todo: dataset info stored separately
        ...
        if not process_only:
            ...
            dataset_info = DatasetInfo.from_dataset(dataset)
            self.dataset_info = dataset_info
    ...
    def build_model(self):
        ...
        begin = time.time()
        net = Net(
                  index_to_Z=self.dataset_info.index_to_Z.tolist(),
    ...
    def save_model(self, src_path):
        ...
        index_to_Z = self.net_out_info.dataset_info.index_to_Z
        num_species = len(index_to_Z)
        with open(os.path.join(src_path, 'build_model.py'), 'w') as f:
            pf = lambda x: print(x, file=f)
            pf(f"from deephe3_1 import Net")
            pf(f"net = Net(")
            pf(f"    index_to_Z={index_to_Z.tolist()},")
    ...
    def load_model(self, src_path, device='cpu'):
        ...
        if self.dataset_info is not None:
            msg = 'Network output incompatible with dataset'
            assert net_out_info_o.dataset_info.contains(self.dataset_info), msg
        # Do we really have to check consistency of net_out_info?
    ...
    def init_normalize(self, net, loader):
        print("Initializing the normalize term")
        begin = time.time()
        construct_kernel = self.construct_kernel

        selfloop_mu = []
        for batch in loader:
            data_net_out = construct_kernel.get_net_out(batch.label.to(device=construct_kernel.device))
            selfloop = (torch.abs(batch["edge_attr"][:, 0]) < 1e-7).to(device=construct_kernel.device)
            node_index = net.Z_to_index.to(batch.x.device)[batch.x]
            index_elem = (node_index[batch.edge_index[0]].to(device=construct_kernel.device))[selfloop]
            selfloop_mu.append(scatter(data_net_out[selfloop][:, net.mu_inv_extract], index_elem, dim=0, dim_size=net.num_species, reduce='mean'))
        #TODO: element not in batch
        #TODO: if 可学习/初始化mu/初始化sigma/初始化非selfloop
        selfloop_mu = torch.stack(selfloop_mu).mean(dim=0)
        print(f"Initializing selfloop_mu with\n{selfloop_mu}")

        # init_sigma = []
        model_state_dict = net.state_dict()
        init_mu = model_state_dict["mu"]
        init_mu[:net.num_species] = selfloop_mu
        model_state_dict["mu"] = init_mu
        net.load_state_dict(model_state_dict)
        # net.sigma.requires_grad_(False)
        # net.mu.requires_grad_(False)
        print(f'Finished initialize, cost {time.time() - begin:.2f} seconds.')

    def register_constructor(self, device='cpu'):
        # requires: net_out_info, train_config which target is already set
        ...
        construct_kernel = e3TensorDecomp(config.net_out_irreps, 
                                          ...
                                          default_dtype_torch=torch.get_default_dtype(), 
                                          spinful=self.net_out_info.dataset_info.spinful,
                                          no_parity=config.no_parity, 
    ...
    def config_set_target(self, dataset_info, verbose=None):
        o, i, s = dataset_info.orbital_types, dataset_info.index_to_Z, dataset_info.spinful# dataset.info['orbital_types'], dataset.info['index_to_Z'], dataset.info['spinful']
        self.train_config.set_target(o, i, s, verbose)
        return NetOutInfo(self.train_config.target_blocks, dataset_info)
    ...
    def save_test_result(self, batch, H_pred, h5_fp):
        ...
        if stru_id in h5_fp:
            ...
            g['node_attr'] = self.net_out_info.dataset_info.Z_to_index[batch.x] # ! for backward compatibility!
    ...
    def update_hopping(self, H_prev, H_pred, node_attr, edge_index, edge_key, debug=False):
        # requires train_config, net_out_info
        ...
        dataset_info = self.net_out_info.dataset_info
        
        atom_num_orbitals = [sum(map(lambda x: 2 * x + 1, atom_orbital_types)) for atom_orbital_types in dataset_info.orbital_types]
        for index_edge in range(edge_index.shape[1]):
            key_term = str(edge_key[index_edge].tolist())
            i, j = dataset_info.Z_to_index[node_attr[edge_index[:, index_edge]]]
            if key_term not in H_prev.keys():
                fill = 0 if debug else np.nan
                fill_sp = 0 + 0j if debug else np.nan + np.nan * 1j
                if dataset_info.spinful:
            ...
            N_M_str_edge = f'{dataset_info.index_to_Z[i].item()} {dataset_info.index_to_Z[j].item()}'

...
for band_i in range(band_num_each_spin_dft):
    ...
    if trim_gap:
        ax_val.plot(x[y<2], y[y<2], 'r-', linewidth=1.5, zorder=3)
        ax_cond.plot(x[y>-2], y[y>-2], 'r-', linewidth=1.5, zorder=3)
    else:
...
for band_i in range(band_num_each_spin_deeph):
    ...
    if trim_gap:
        ax_val.scatter(x[y<2], y[y<2], c='b', s=5, zorder=4)
        ax_cond.scatter(x[y>-2], y[y>-2], c='b', s=5, zorder=4)
    else:
